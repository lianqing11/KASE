...
from memcached_dataset import McDataset, TeacherDataset
...
def main():
    ...
    train_source_dataset = McDataset(
    ...
    if args.evaluate:
        validate(val_loader, teacher_model, criterion)
        return

    train(train_source_loader, train_target_loader, val_loader,
           student_model, teacher_model, criterion, student_optimizer,
           teacher_optimizer,  lr_scheduler, last_iter+1, tb_logger)

def train(train_source_loader, train_target_loader, val_loader,
          student_model, teacher_model, criterion, student_optimizer,
          teacher_optimizer, lr_scheduler, start_iter, tb_logger):
    ...
    for i, (batch_source, batch_target) in enumerate(zip(train_source_loader, train_target_loader)):
        ...
        input_target1, input_target2, label_target = batch_target
        p = i / len(train_source_loader)
        constant = 2. / (1. + np.exp(-10*p)) -1
        if args.fix_constant == True:
        ...
        label_source = label_source.cuda(async=True)
        input_source = input_source.cuda()
        input_target1 = input_target1.cuda()
        input_target2 = input_target2.cuda()
        input_source_var = torch.autograd.Variable(input_source)
        input_target1_var = torch.autograd.Variable(input_target1)
        input_target2_var = torch.autograd.Variable(input_target2)
        label_source_var = torch.autograd.Variable(label_source)

        # compute output for source data
        source_output, source_output2 = student_model(input_source_var)

        # measure accuracy and record loss
        softmax_source_output = F.softmax(source_output, dim=1)

        #loss for known class
        known_ind = label_source_var != 12
        loss = criterion(softmax_source_output[known_ind], label_source_var[known_ind])

        #loss for unknown class
        loss_entropy = torch.mean(
            torch.mul(softmax_source_output[label_source_var==12], torch.log(softmax_source_output[label_source_var==12])))*12
        #integrate loss_cls and loss_entropy
        loss_sum = loss + args.lambda_entropy*loss_entropy
        uk_label_var = label_source_var.clone()
        uk_label_var[uk_label_var!=12] = 0
        uk_label_var[uk_label_var==12] = 1
        loss_cls_uk =  criterion_uk(source_output2, uk_label_var.view(-1,1).float())
        loss_sum += args.lambda_uk * loss_cls_uk
        #compute output for target data
        stu_out, stu_out2 = student_model(input_target1_var)
        tea_out, tea_out2 = teacher_model(input_target2_var)

        #compute losses
        loss_aug, conf_mask, loss_cls_bal = \
            utils.compute_aug_loss_enp(stu_out, tea_out, args.aug_thresh, F.softmax(tea_out, dim=1).detach(), args.cls_balance, args)
       
        conf_mask_count = torch.sum(conf_mask)/ args.batch_size
        loss_aug = torch.mean(loss_aug)
        loss_sum += args.lambda_aug * loss_aug
        #compute accuracy
        prec1, prec5 = accuracy_2(softmax_source_output.data, source_output2.data, label_source, 0.5, topk=(1, 5))

        eval_output.append(F.softmax(source_output, dim=1).cpu().data.numpy())
        ...
        losses_cls_uk.update(loss_cls_uk.item())
        if loss_cls_bal is not None:
            losses_bal.update(loss_cls_bal.item())
        confs_mask_count.update(conf_mask_count.item())
        ...
        if curr_step % args.print_freq == 0 :
            ...
            tb_logger.add_scalar('loss_bal', losses_bal.avg, curr_step)
            tb_logger.add_scalar('mask count', confs_mask_count.avg, curr_step)
            tb_logger.add_scalar('loss_aug', losses_aug.avg, curr_step)
            tb_logger.add_scalar('loss_cls_uk', losses_cls_uk.avg, curr_step)
            tb_logger.add_scalar('loss_entorpy', losses_entropy.avg, curr_step)
            tb_logger.add_scalar('loss_bal_uk', losses_bal_uk.avg, curr_step)
            tb_logger.add_scalar('loss_aug_uk', losses_aug_uk.avg, curr_step)
            logger.info('Iter: [{0}/{1}]\t'
                  'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                  'Data {data_time.val:.3f} ({data_time.avg:.3f})\t'
                  'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                  'Loss aug {loss_aug.val:.7f} ({loss_aug.avg:.7f})\t'
                  'Loss bal {loss_bal.val:.6f} ({loss_bal.avg:.6f})\t'
                  'Loss entropy {loss_entropy.val:.3f} ({loss_entropy.avg:.3f})\t'
                  'Loss uk {loss_cls_uk.val:.4f} ({loss_cls_uk.val:.4f})\t'
                  'Loss bal uk {loss_bal_uk.val:.4f} ({loss_bal_uk.avg:.4f})\t'
                  'Loss aug uk {loss_aug_uk.val:.4f} ({loss_aug_uk.avg:.4f})\t'
                  'mask count{conf_mask.val:.5f} ({conf_mask.avg:.7f})\t'
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})\t'
                  'LR {lr:.6f}'.format(
                   curr_step, len(train_source_loader), batch_time=batch_time,
                   data_time=data_time, loss=losses, loss_aug = losses_aug, loss_entropy=losses_entropy, loss_cls_uk=losses_cls_uk,
                   loss_bal=losses_bal, conf_mask = confs_mask_count, loss_aug_uk=losses_aug_uk, loss_bal_uk=losses_bal_uk,
                   top1=top1, top5=top5,
                   lr=current_lr))
        ...
        if (curr_step+1)%args.val_freq == 0 :
            ...
            evaluator = utils.PredictionEvaluator_2(eval_target, 13)
            ...
            val_loss, prec1, prec5, mean_aug_class_acc, aug_cls_acc  = validate(val_loader, teacher_model, criterion)
            if not tb_logger is None:
                ...
                for cls in range(args.num_classes):
                    tb_logger.add_scalar('acc_val_' + args.class_name[cls], aug_cls_acc[cls], curr_step)
            logger.info("evaluate step mean:{} class:{}".format(mean_aug_class_acc, aug_cls_acc))
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        known_id = target_var !=12
        #loss for known class
        loss = criterion(softmax_output[known_id], target_var[known_id]) 

        prec1, prec5 = accuracy_2(softmax_output.data, output1.data, target,0.5, topk=(1, 5))
        eval_target.append(target.cpu().data.numpy())
        eval_output.append(F.softmax(output, dim=1).cpu().data.numpy())
    ...
    mean_aug_class_acc, aug_cls_acc = evaluator.evaluate(eval_output, eval_uk, 0.5)
    for i in range(9):
        t_clss_acc, t_aug_cls_acc = evaluator.evaluate(eval_output, eval_uk, (i+1)*0.1)
        print("epslion {}, mean_aug_class_acc {}, aug_cls_acc {}".format((i+1)*0.1, t_clss_acc, t_aug_cls_acc))
    model.train(mode=True)

    return losses.avg, top1.avg, top5.avg, mean_aug_class_acc, aug_cls_acc
