...
def train(train_source_loader, train_target_loader, val_loader,
    ...
    for i, (batch_source, batch_target) in enumerate(zip(train_source_loader, train_target_loader)):
        ...
        known_ind = label_source != args.num_classes-1
        ...
        eval_uk.append(F.sigmoid(source_output2).cpu().data.numpy())
        ...
        if (curr_step+1)%args.val_freq == 0 :
            ...
            val_loss, prec1, prec5, mean_aug_class_acc, aug_cls_acc  = validate(val_loader, student_model, criterion)
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        prec1, prec5 = accuracy_2(softmax_output.data,
                                  F.sigmoid(output1).data,
                                  target, 0.5, topk=(1, 5))
        #losses.update(loss.item())
        eval_target.append(target.cpu().data.numpy())
        eval_output.append(softmax_output.data.numpy())
        eval_uk.append(F.sigmoid(output1).data.numpy())
